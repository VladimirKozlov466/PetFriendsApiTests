...
def get_key(email=valid_email, password=valid_password):
    ...
    # assert status == 200 # Добавляет проверку в каждый тест (поэтому в позитивных тестах писать ее внутри теста уже не надо)

#Функция генерирует строку нужной длинны символов
...
def test_getAllPetsWith_negative_filer(get_key, filter):  # filter available values : my_pets
...
def test_getAllPetsWithValidKey(get_key, filter):  # filter available values : my_pets
    status, result = pf.get_list_of_pets(get_key, filter)
    assert status == 200
...
@pytest.mark.add
def test_add_new_pet_with_valid_data(get_key, name='Матроскин', animal_type='гулящий', age='4',
...
def test_successful_delete_self_pet(get_key):
    ...
    pf.add_new_pet(get_key, "Дядя Федор", "кот", "7", pet_photo)
...
@pytest.mark.update
def test_successful_update_self_pet_info(get_key, name='Василевс', animal_type='Котяра', age=11):
...
@pytest.mark.add
def test_create_new_pet_simple_with_valid_data(get_key, name='Шарик', animal_type='Котопес', age='9'):
    """Проверяем что можно добавить питомца с корректными данными без фото питомца"""

    # Добавляем питомца
    status, result = pf.create_new_pet_simple(get_key, name, animal_type, age)
...
@pytest.mark.update
def test_successful_add_photo_of_pet(get_key, pet_photo='images/cat1.jpg'):
    ...
    pf.create_new_pet_simple(get_key, 'Новый', 'неопределенный', '7')
...
@pytest.mark.add
def test_add_new_pet_with_valid_data_and_big_photo(get_key, name='БольшойКот', animal_type='тестировочный', age='50',
...
@pytest.mark.skip
def test_create_new_pet_simple_with_negative_age(get_key, name='Бобик', animal_type='Не_рожденный', age=-9):
...
@pytest.mark.skip
def test_create_new_pet_simple_with_too_big_age(get_key, name='Крокодил', animal_type='мумия', age=1000):
...
@pytest.mark.add_neg
def test_create_new_pet_simple_without_data(get_key, name='', animal_type='', age=''):
...
@pytest.mark.update_neg
def test_successful_update_of_pet_name_by_changing_to_empty_name(get_key, name='', animal_type='собачка', age=22):
...
@pytest.mark.add_neg
def test_create_new_pet_simple_with_text_in_age_field(get_key, name='Енот', animal_type='Полоскун', age='млгнпм'):
...
@pytest.mark.add_neg
def test_create_new_pet_simple_with_unsafe_data(get_key, name='SELECT * FROM users', animal_type='SELECT * FROM users',
...
@pytest.mark.add_neg
def test_add_new_pet_with_special_symbols_in_data(get_key, name='@#$%/?^+', animal_type='%/?^&*', age='@!$#\%^*',
                                                  pet_photo='images/cat1.jpg'):
...
@pytest.mark.add_neg
def test_create_new_pet_simple_with_long_values_of_data(get_key,
